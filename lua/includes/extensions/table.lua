...
--[[---------------------------------------------------------
	Name: table_sortDesc( table )
	Desc: Like Lua's default sort, but descending
-----------------------------------------------------------]]
function table_sortDesc( t )
    return table_sort( t, function( a, b )
        return a > b
    end )
end

--[[---------------------------------------------------------
	Name: table_sortByKey( table )
	Desc: Returns a table sorted numerically by Key value
-----------------------------------------------------------]]
function table_sortByKey( t, desc )
    local temp = {}
...
--[[---------------------------------------------------------
	Name: table_sortByMember( table )
	Desc: Sorts table by named member
-----------------------------------------------------------]]
function table.SortByMember( Table, MemberKey, bAsc )
    ...
    table_sort( Table, function( a, b )
        return TableMemberSort( a, b, MemberKey, bAsc or false )
    end )
end
table_SortByMember = table.SortByMember
...
function SortedPairs( pTable, Desc )
    ...
    if Desc then
        table_sort( sortedTbl, function( a, b )
            return a.key > b.key
        end )
    else
        table_sort( sortedTbl, function( a, b )
...
function SortedPairsByMemberValue( pTable, pValueName, Desc )
    ...
    table_SortByMember( sortedTbl, "member", not Desc )
...
table["GetFirstValue"] = function( t )
    local _, v = next( t )
...
table["GetLastKey"] = function( t )
    local k = next( t, table_Count( t ) - 1 )

    return k
end

table["GetLastValue"] = function( t )
    local _, v = next( t, table_Count( t ) - 1 )

    return v
end
local table_GetLastValue = table["GetLastValue"]
...
table["FindNext"] = function( tab, val )
    ...
    return table_GetFirstValue( tab )
end

table["FindPrev"] = function( tab, val )
    local last = table_GetLastValue( tab )
...
function table.MemberValuesFromKey( tab, key )
    ...
    for _, v in pairs( tab ) do
        if istable( v ) and v[key] ~= nil then
            res[#res + 1] = v[key]
        end
    end
...
table["ForEach"] = function( tab, funcname )
    for k, v in pairs( tab ) do
        funcname( k, v )
    end
end
